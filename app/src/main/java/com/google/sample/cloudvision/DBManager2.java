...
/**
 * Created by cx61 on 2017-06-09.
 */

public class DBManager2 extends SQLiteOpenHelper {
    public DBManager2(Context context, String name, SQLiteDatabase.CursorFactory factory,
                     int version) {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String sql = "CREATE TABLE member" + "(num INTEGER PRIMARY KEY AUTOINCREMENT," +
                " uri text, date text, " +
                " Dish text ,calorie integer, Carbohydrate integer, protein integer, fat integer );";
        //sql문 실행하기
        db.execSQL(sql);
        Log.d(TAG, "테이블 생성");
    }
    ...
    protected void input(String uri, String date,  String dish, int cal, int car, int pro, int fat){
        rwDB = getWritableDatabase();
        rwDB.execSQL("INSERT INTO member VALUES(NULL,'" + uri +"','"+ date  +"','" + dish +"','" + cal +"','" + car +"','" + pro +"','"+ fat + "');");
        Log.d(TAG, "값 입력");
    }
